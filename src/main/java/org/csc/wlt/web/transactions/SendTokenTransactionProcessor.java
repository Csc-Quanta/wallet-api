...
import org.csc.wlt.helper.validator.TxSendTokenValidator;

@NActorProvider
@Slf4j
@Data
@EqualsAndHashCode(callSuper=false)
public class SendTokenTransactionProcessor extends TransactionProcessorSessionModules<Wallet.BaseData>{

    @Override
    public String[] getCmds() {
        return new String[]{Wallet.PWLTCommand.TTT.name()};
    }
    ...
    public void onPBPacket(final FramePacket pack, final Wallet.BaseData pb, final CompleteHandler handler) {
        ...
        log.info("SendTokenTransactionProcessor ret:{}",ret.build().toString());
    ...
    protected boolean verifyRequestData(Wallet.MultiTransactionImpl txImpl, Wallet.RespCreateTransaction.Builder ret) {
        ...
        new TxSendTokenValidator().isVerify(txImpl, ret);
