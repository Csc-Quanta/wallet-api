...
import org.csc.wlt.helper.validator.TxDoContractValidator;

@NActorProvider
@Slf4j
@Data
@EqualsAndHashCode(callSuper=false)
public class CallContractTransactionProcessor extends TransactionProcessorSessionModules<Wallet.BaseData>{

    @Override
    public String[] getCmds() {
        return new String[]{Wallet.PWLTCommand.TCC.name()};
    }
    ...
    public void onPBPacket(final FramePacket pack, final Wallet.BaseData pb, final CompleteHandler handler) {
        ...
        log.info("CallContractTransactionProcessor ret:{}",ret.build().toString());
    ...
    protected boolean verifyRequestData(Wallet.MultiTransactionImpl txImpl, Wallet.RespCreateTransaction.Builder ret) {
        ...
        new TxDoContractValidator().isVerify(txImpl, ret);
