...
import org.csc.wlt.helper.validator.TxCreateTokenValidator;
...
public class CreateTokenTransactionProcessor extends TransactionProcessorSessionModules<Wallet.BaseData>{

    @Override
    public String[] getCmds() {
        return new String[]{Wallet.PWLTCommand.TBT.name()};
    }
    ...
    public void onPBPacket(final FramePacket pack, final Wallet.BaseData pb, final CompleteHandler handler) {
        ...
        log.info("CreateTokenTransactionProcessor ret:{}",ret.build().toString());
    ...
    protected boolean verifyRequestData(Wallet.MultiTransactionImpl txImpl, Wallet.RespCreateTransaction.Builder ret) {
        ...
        new TxCreateTokenValidator().isVerify(txImpl, ret);
