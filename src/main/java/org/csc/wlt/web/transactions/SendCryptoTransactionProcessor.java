...
import org.csc.wlt.helper.validator.TxSendCryptoValidator;
@NActorProvider
@Slf4j
@Data
@EqualsAndHashCode(callSuper=false)
public class SendCryptoTransactionProcessor extends TransactionProcessorSessionModules<Wallet.BaseData>{

    @Override
    public String[] getCmds() {
        return new String[]{Wallet.PWLTCommand.TRT.name()};
    }
    ...
    public void onPBPacket(final FramePacket pack, final Wallet.BaseData pb, final CompleteHandler handler) {
        ...
        log.info("SendCryptoTransactionProcessor ret:{}",ret.build().toString());
    ...
    protected boolean verifyRequestData(Wallet.MultiTransactionImpl txImpl, Wallet.RespCreateTransaction.Builder ret) {
        ...
        new TxSendCryptoValidator().isVerify(txImpl, ret);
